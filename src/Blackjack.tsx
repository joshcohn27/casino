...
const CARD_DEAL_DURATION = 0.44;
const CARD_DEAL_STAGGER  = 0.10;
...
// Fixed px — reliable, no clamp weirdness
                ...
                <AnimatePresence initial={false}>
                    {cards.map((card, i) => (
                        <motion.div
                            ...
                            transition={{ duration: CARD_DEAL_DURATION, ease: [0.22, 1, 0.36, 1], delay: i * CARD_DEAL_STAGGER }}
                        >
...
function HandBox({ hand, visibleCount, bet, index, isActive, stage, dealerTotal, dealerBusted }: {
    hand: Card[]; visibleCount: number; bet: number; index: number; isActive: boolean;
    stage: Stage; dealerTotal: number; dealerBusted: boolean;
}) {
    const scoreCards = stage === "done" ? hand : hand.slice(0, Math.min(visibleCount, hand.length));
    const t    = scoreCards.length ? total(scoreCards) : 0;
    const bust = scoreCards.length === hand.length && t > 21;
    ...
    return (
        ...
        >
            ...
            <div className="text-sm font-bold text-amber-100">
                {scoreCards.length ? `${t}${isSoft(scoreCards) ? " soft" : ""}${bust ? " · Bust" : ""}` : "—"}
            </div>
...
// Bottom bar — chip tray left, action buttons center-right
function BlackjackBar({
    stage, isShuffling, bet, bankroll, canDouble, canSplit,
    ...
    canDouble: boolean; canSplit: boolean;
    ...
    return (
        <div className="grid grid-cols-[1fr_auto] items-center gap-4 border-t border-white/10 bg-black/55 px-4 py-3 backdrop-blur-xl">
            {/* Chip tray — always left, disabled mid-hand */}
            <ChipTray
            ...
            {/* Stage-dependent buttons */}
            <div className="flex shrink-0 items-center gap-2">
                <AnimatePresence mode="popLayout" initial={false}>
                    {/* Betting / Done controls */}
                    {isDone && (
...
export default function BlackjackTable({ bankroll, setBankroll }: Props) {
    ...
    const revealHandCount = (idx: number, count: number) => {
        setHandVisibleCounts((prev) => { const next = [...prev]; next[idx] = count; return next; });
    };
    ...
    const resetTableForBetting = (options?: { clearChips?: boolean; message?: string }) => {
        setDealer([]); setHands([]); setHandVisibleCounts([]); setBets([]);
        setActive(0); setDealerRevealedCount(1); setRoundReturned(0); setRoundNet(0);
        setStage("betting");
        if (options?.clearChips) { setBet(0); setChipStack([]); }
    ...
    const finishRoundWithoutDealer = async (finalHands: Card[][], wagersInPlay: number[] = bets) => {
        ...
        setRoundReturned(0);
        setRoundNet(-wagered);
        if (dealer.length > 0) { setDealerRevealedCount(dealer.length); await wait(DEALER_REVEAL_PAUSE); }
        setStage("done");
        setMessage(allBusted ? "All hands bust." : "Round complete.");
    };
    ...
    const moveToNextHand = async (
        ...
        if (nextIdx < nextHands.length) {
            ...
            if (nextTotal === 21) { setMessage(`Hand ${nextIdx + 1} makes 21.`);  await wait(320); await moveToNextHand(dealt.hands, dealt.deck, nextBets, nextIdx); return; }
            if (nextTotal >  21)  { setMessage(`Hand ${nextIdx + 1} busts.`);     await wait(320); await moveToNextHand(dealt.hands, dealt.deck, nextBets, nextIdx); return; }
            return;
        }
    ...
    const split = async () => {
        ...
        try {
            ...
            setHands(nextHands);
            setHandVisibleCounts([...handVisibleCounts.slice(0, active), 1, 1, ...handVisibleCounts.slice(active + 1)]);
            setBets(nextBets);
            ...
            if (activeTotal === 21) { setMessage(`Hand ${active + 1} makes 21.`); await wait(320); await moveToNextHand(dealt.hands, dealt.deck, nextBets, active); return; }
            if (activeTotal >  21)  { setMessage(`Hand ${active + 1} busts.`);    await wait(520); await moveToNextHand(dealt.hands, dealt.deck, nextBets, active); }
        } finally {
    ...
    const deal = async () => {
        ...
        if (stage === "done") resetTableForBetting({ clearChips: false, message: "Dealing next hand..." });

        const override = dealBetRef.current;
        dealBetRef.current = null;
        const wager = Math.max(MIN_BET, Math.floor((override ?? bet) / 5) * 5);

        if (bet < MIN_BET && override === null) { setMessage("Place at least $5 to deal."); return; }
        ...
        const playerCard1 = nextDeck[0], playerCard2 = nextDeck[1];
        const dealerCard1 = nextDeck[2], dealerCard2 = nextDeck[3];
        nextDeck = nextDeck.slice(4);

        setDeck(nextDeck); setDealer([]); setHands([[]]); setHandVisibleCounts([0]);
        setBets([wager]); setActive(0); setDealerRevealedCount(1);
    ...
    const hit = async () => {
        ...
        try {
            ...
            if (t === 21) { setMessage(`Hand ${active + 1} makes 21.`); await wait(420); await moveToNextHand(nextHands, nextDeck, bets); return; }
            if (t >  21)  { setMessage(`Hand ${active + 1} busts.`);    await wait(520); await moveToNextHand(nextHands, nextDeck, bets); }
    ...
    const stay = async () => {
        if (isResolvingAction) return;
        setIsResolvingAction(true);
        try { await wait(240); await moveToNextHand(hands, deck, bets); }
        finally { setIsResolvingAction(false); }
    };
    ...
    const doubleDown = async () => {
        ...
        try {
            ...
            await wait(CARD_VALUE_REVEAL_DELAY);
            revealHandCount(active, nextHands[active].length);
            if (total(nextHands[active]) > 21) { setMessage(`Hand ${active + 1} busts after doubling.`); await wait(520); }
    ...
    const clearBet = () => {
        if (stage === "done")    { resetTableForBetting({ clearChips: true }); return; }
        if (stage === "betting") { setBet(0); setChipStack([]); }
    };

    const changeBet = () => {
        if (stage !== "done") return;
        resetTableForBetting({ clearChips: false, message: "Adjust your bet, then press Deal." });
    };
    ...
    const canDouble =
        stage === "player" && activeHand.length === 2 && bankroll >= (bets[active] ?? 0);
    ...
    return (
        <>
            ...
            >
                {/* ── Clean layout from Doc 20 — no min-h, no justify-between ── */}
